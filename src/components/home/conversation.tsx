...
const Conversation = ({ conversation }: { conversation: any }) => {
  ...
  // State to control confirmation dialog visibility
  const [showConfirmation, setShowConfirmation] = useState(false);

  // Handles exiting the conversation after user confirms
  const handleExitConversation = async () => {
    try {
      await exitConversation({ conversationId: conversation._id });
      // Optionally, reset selected conversation if the current one is being exited
      if (selectedConversation?._id === conversation._id) {
        setSelectedConversation(null);
      }
      setShowConfirmation(false); // Close the modal after exiting
    } catch (error) {
      ...
      >
        ...
        <div className="flex text-left gap-7 mr-5">
          <X
            ...
            onClick={(e) => {
              e.stopPropagation(); // Prevent triggering the click that selects the conversation
      ...
      {showConfirmation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-secondary p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Are you sure you want to exit this conversation?</h2>
            <div className="flex gap-4 justify-center">
              <button
                className="px-4 py-2 bg-indigo-primary rounded-md hover:bg-indigo-300"
                onClick={() => setShowConfirmation(false)} // Cancel exit
              >
                Cancel
              </button>
              <button
                className="px-4 py-2 bg-indigo-primary rounded-md hover:bg-indigo-300"
                onClick={handleExitConversation} // Confirm exit
              >
                Exit
              </button>
            </div>
